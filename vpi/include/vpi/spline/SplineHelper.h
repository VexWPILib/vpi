...
class SplineHelper {
 ...
 private:
  static Spline<3>::ControlVector CubicControlVector(double scalar,
                                                     const Pose2d& point) {
    return {{point.X().convert(meter), scalar * point.Rotation().Cos()},
            {point.Y().convert(meter), scalar * point.Rotation().Sin()}};
  }

  static Spline<5>::ControlVector QuinticControlVector(double scalar,
                                                       const Pose2d& point) {
    return {{point.X().convert(meter), scalar * point.Rotation().Cos(), 0.0},
            {point.Y().convert(meter), scalar * point.Rotation().Sin(), 0.0}};
  }
