...
std::vector<CubicHermiteSpline> SplineHelper::CubicSplinesFromControlVectors(
  ...
  if (waypoints.size() > 1) {
    waypoints.emplace(waypoints.begin(),
                      Translation2d{xInitial[0] * meter,
                                    yInitial[0] * meter});
    waypoints.emplace_back(
        Translation2d{xFinal[0] * meter, yFinal[0] * meter});
    ...
    */
    ...
    dx.emplace_back(3 * (waypoints[2].X().convert(meter) - waypoints[0].X().convert(meter)) -
                    xInitial[1]);
    dy.emplace_back(3 * (waypoints[2].Y().convert(meter) - waypoints[0].Y().convert(meter)) -
                    yInitial[1]);
    if (waypoints.size() > 4) {
      for (size_t i = 1; i <= waypoints.size() - 4; ++i) {
        ...
        dx.emplace_back(
            3 * (waypoints[i + 2].X().convert(meter) - waypoints[i].X().convert(meter)));
        dy.emplace_back(
            3 * (waypoints[i + 2].Y().convert(meter) - waypoints[i].Y().convert(meter)));
      }
    }
    dx.emplace_back(3 * (waypoints[waypoints.size() - 1].X().convert(meter) -
                         waypoints[waypoints.size() - 3].X().convert(meter)) -
                    xFinal[1]);
    dy.emplace_back(3 * (waypoints[waypoints.size() - 1].Y().convert(meter) -
                         waypoints[waypoints.size() - 3].Y().convert(meter)) -
                    yFinal[1]);
    ...
    for (size_t i = 0; i < fx.size() - 1; ++i) {
      ...
      const CubicHermiteSpline spline{
          {waypoints[i].X().convert(meter), fx[i]},
          {waypoints[i + 1].X().convert(meter), fx[i + 1]},
          {waypoints[i].Y().convert(meter), fy[i]},
          {waypoints[i + 1].Y().convert(meter), fy[i + 1]}};
    ...
    std::array<double, 2> midXControlVector{waypoints[0].X().convert(meter), xDeriv};
    std::array<double, 2> midYControlVector{waypoints[0].Y().convert(meter), yDeriv};
...
SplineHelper::CubicControlVectorsFromWaypoints(
  ...
  if (interiorWaypoints.empty()) {
    scalar = 1.2 * start.Translation().Distance(end.Translation()).convert(meter);
  } else {
    scalar =
        1.2 * start.Translation().Distance(interiorWaypoints.front()).convert(meter);
  }
  const auto initialCV = CubicControlVector(scalar, start);
  if (!interiorWaypoints.empty()) {
    scalar = 1.2 * end.Translation().Distance(interiorWaypoints.back()).convert(meter);
  }
...
std::vector<QuinticHermiteSpline> SplineHelper::QuinticSplinesFromWaypoints(
  ...
  for (size_t i = 0; i < waypoints.size() - 1; ++i) {
    ...
    const auto scalar =
        1.2 * p0.Translation().Distance(p1.Translation()).convert(meter);
